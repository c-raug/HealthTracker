...
- **`CustomFoodForm`** (`components/nutrition/CustomFoodForm.tsx`): serving size is quantity + unit picker (g, oz, qty). Calories auto-computed from macros via `useEffect`; manual override shows an `Alert` warning. Accepts optional `initialFood?: CustomFood` and `mode?: 'create' | 'edit'` props — in edit mode pre-populates all fields, dispatches `UPDATE_CUSTOM_FOOD` instead of `ADD_CUSTOM_FOOD`, and shows "Edit Custom Food" / "Save Changes" labels.
