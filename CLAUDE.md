...
- **`MacroSection`** (`components/settings/MacroSection.tsx`): accepts `goalCalories: number | null` and `activityAdjusted?: boolean` props from `settings.tsx`. Displays gram equivalents below each preset button and custom % input. Shows `—g` when no goal calories are available. Shows an ⓘ icon (taps to `Alert.alert()`) explaining that grams factor in profile, weight, and 7-day average activity. `settings.tsx` passes `adjustedGoalCalories = TDEE + Math.round(avgActivityCalories)` where `avgActivityCalories` is the 7-day mean of daily `caloriesBurned` filtered by `activityMode` (same logic as `nutrition.tsx`).
...
- **`CreateMealFlow`** (`components/nutrition/CreateMealFlow.tsx`): food search step uses a `SectionList` with a "Pinned" section (foods where `customFoods.find(f => f.id === item.id)?.pinned`) and a "My Foods" section — no management buttons in this context, rows are tap-to-select only. Both sections are filtered simultaneously by the search query.
...
- **Settings screen** (`app/(tabs)/settings.tsx`): reads `focusActivityMode` search param via `useLocalSearchParams()`; when present, auto-expands the "Goals & Calorie Target" collapsible and scrolls the `ScrollView` to it (150ms delay to allow expansion). GoalsSection field order: Weight Goal (drum scroll wheel) → Activity Tracking Mode → Activity Level (only shown when mode is `'auto'`) → Fitness Goal. The Weight Goal drum uses the same `ScrollView` snap pattern as the hour/minute drums in `activities.tsx` (constants: `ITEM_HEIGHT=44`, `VISIBLE_ITEMS=5`, `PAD_COUNT=2`); scroll position is initialized on mount via a `useRef<ScrollView>` + 50ms `setTimeout`.
