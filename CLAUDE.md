...
- **Settings screen** (`app/(tabs)/settings.tsx`): reads `focusActivityMode` search param via `useLocalSearchParams()`; when present, auto-expands the "Goals & Calorie Target" collapsible and scrolls the `ScrollView` to it (150ms delay to allow expansion). GoalsSection field order: Weight Goal → Activity Tracking Mode → Activity Level (only shown when mode is `'auto'`) → Fitness Goal.
