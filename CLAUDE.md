...
- **Settings screen** (`app/(tabs)/settings.tsx`): reads `focusActivityMode` search param via `useLocalSearchParams()`; when present, auto-expands the "Goals & Calorie Target" collapsible and scrolls the `ScrollView` to it (150ms delay to allow expansion). GoalsSection field order: Weight Goal (drum scroll wheel) → Activity Tracking Mode → Activity Level (only shown when mode is `'auto'`) → Fitness Goal. The Weight Goal drum uses the same `ScrollView` snap pattern as the hour/minute drums in `activities.tsx` (constants: `ITEM_HEIGHT=44`, `VISIBLE_ITEMS=3`, `PAD_COUNT=1`); scroll position is initialized on mount via a `useRef<ScrollView>` + 50ms `setTimeout`. Section order: Profile → Goals & Calorie Target → Accent Color → Units → Macros → Data Backup → Send Feedback.
...
- **`FeedbackSection`** (`components/settings/FeedbackSection.tsx`): multi-line `TextInput` (min 5–6 visible lines) + disabled-until-filled Submit button; on submit opens a `mailto:` link via `Linking.openURL` pre-filled with the message; shows "Thanks for your feedback!" confirmation for 4 s; clears input after submit. No new state in context — all local component state. Recipient email is the `FEEDBACK_EMAIL` const at the top of the file.
