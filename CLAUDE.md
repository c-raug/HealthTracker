...
- **Collapsible sections**: use `useState(false)` + `TouchableOpacity` header + `Ionicons` chevron-down/chevron-forward + conditional render `{!collapsed && <Content />}`. See `components/nutrition/MealCategory.tsx` for the canonical pattern. Settings uses this for Profile, Goals & Calorie Target, and Macros.
...
- **`PortionSelector`** (`components/nutrition/PortionSelector.tsx`): two drum scroll wheels (whole number 0–250 + fraction in ⅛ increments) + live macro preview. Used in `AddFoodTab` (before adding), `CreateMealFlow` and `EditMealFlow` (before adding food to a saved meal), and `FoodItem` (bottom-sheet edit modal after adding).
...
- **Settings screen** (`app/(tabs)/settings.tsx`): reads `focusActivityMode` search param via `useLocalSearchParams()`; when present, auto-expands the "Goals & Calorie Target" collapsible and scrolls the `ScrollView` to it (150ms delay to allow expansion). Wrapped in `KeyboardAvoidingView` (behavior `padding` on iOS, `height` on Android) with `keyboardShouldPersistTaps="handled"` on the inner `ScrollView` so the Feedback input stays visible above the keyboard. GoalsSection field order: Weight Goal (drum scroll wheel) → Activity Tracking Mode → Activity Level (only shown when mode is `'auto'`). Section order: Profile → Goals & Calorie Target → Macros → Accent Color → Units → Data Backup → Send Feedback.
- **`ThemeColorPicker`** (`components/settings/ThemeColorPicker.tsx`): horizontal row of 6 color swatches from `ACCENT_PRESETS`; selected swatch shows a checkmark; dispatches `SET_THEME_COLOR` on tap. Reads `preferences.themeColor` (defaults to green preset if unset). Rendered as a card in Settings.
- **`FeedbackSection`** (`components/settings/FeedbackSection.tsx`): multi-line `TextInput` (min 5–6 visible lines) + disabled-until-filled Submit button; on submit does a fire-and-forget `fetch()` POST (`mode: 'no-cors'`) to Google Forms `formResponse` endpoint; shows "Thanks for your feedback!" confirmation for 4 s and clears input. Shows "Sending…" label while in-flight. No new state in context — all local component state. `GOOGLE_FORM_ID` and `GOOGLE_FORM_ENTRY` constants at the top of the file must be filled in with the form's `e/` ID and `entry.XXXXXXXXXX` field ID (see the comment block in the file for how to extract them from the form source).
